#!/usr/bin/env python3
"""Application signing and notarization module for Bharat Browser (macOS)"""
...
def get_browseros_server_binary_info(component_path: Path) -> Optional[Dict[str, str]]:
    """Return metadata for known Bharat Browser Server binaries, if applicable."""
...
class MacOSSignModule(CommandModule):
    ...
    def execute(self, ctx: Context) -> None:
        ...
        if not env_ok:
            log_warning("macOS signing environment not configured. Skipping signing.")
            return

        log_info("=" * 70)
        log_info("🚀 Starting signing process for Bharat Browser...")
        log_info("=" * 70)
