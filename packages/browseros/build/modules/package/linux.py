#!/usr/bin/env python3
"""Linux packaging module for Bharat Browser (AppImage and .deb)"""
...
"""
    ...
    desktop_file = Path(join_paths(apps_dir, "bharat-browser.desktop"))
...
def copy_icon(ctx: Context, icons_dir: Path) -> bool:
    ...
    for size in [16, 22, 24, 32, 48, 64, 128, 256]:
        ...
        if icon_src.exists():
            icon_dest = Path(join_paths(icons_dir, f"{size}x{size}", "apps", "bharat-browser.png"))
...
def prepare_appdir(ctx: Context, appdir: Path) -> bool:
    ...
    app_root = join_paths(appdir, "opt", "bharat-browser")
    ...
    desktop_file = create_desktop_file(
        apps_dir, f"/opt/bharat-browser/{ctx.BROWSEROS_APP_NAME}"
    )
    ...
    appdir_desktop = Path(join_paths(appdir, "bharat-browser.desktop"))
    shutil.copy2(desktop_file, appdir_desktop)
    desktop_content = appdir_desktop.read_text()
    desktop_content = desktop_content.replace(
        f"Exec=/opt/bharat-browser/{ctx.BROWSEROS_APP_NAME} %U", "Exec=AppRun %U"
    )
    ...
    if icon_src.exists():
        appdir_icon = Path(join_paths(appdir, "bharat-browser.png"))
...
THIS="$(readlink -f "${{0}}")"
HERE="$(dirname "${{THIS}}")"
export LD_LIBRARY_PATH="${{HERE}}"/opt/bharat-browser:$LD_LIBRARY_PATH
export CHROME_WRAPPER="${{THIS}}"
"${{HERE}}"/opt/bharat-browser/{ctx.BROWSEROS_APP_NAME} "$@"
"""
...
def create_launcher_script(ctx: Context, bin_dir: Path) -> None:
    """Create launcher script in /usr/bin/bharat-browser."""
    bin_dir.mkdir(parents=True, exist_ok=True)

    launcher_content = f"""#!/bin/sh
# Bharat Browser launcher script
export LD_LIBRARY_PATH=/usr/lib/bharat-browser:$LD_LIBRARY_PATH
exec /usr/lib/bharat-browser/{ctx.BROWSEROS_APP_NAME} "$@"
"""

    launcher_path = Path(join_paths(bin_dir, "bharat-browser"))
...
def create_control_file(ctx: Context, debian_dir: Path) -> None:
    ...
    control_content = f"""Package: bharat-browser
Version: {version}
Section: web
Priority: optional
...
Recommends: apparmor
Maintainer: Bharat Browser Team <support@bharatbrowser.com>
Homepage: https://www.bharatbrowser.com/
Description: Bharat Browser - The open source agentic browser
 Bharat Browser is a privacy-focused web browser built on Chromium,
...
# Post-installation script for Bharat Browser
set -e

# Set SUID bit on chrome_sandbox for sandboxing support
if [ -f /usr/lib/bharat-browser/chrome_sandbox ]; then
    chmod 4755 /usr/lib/bharat-browser/chrome_sandbox
fi
...
if [ -d /etc/apparmor.d ] && command -v apparmor_parser >/dev/null 2>&1; then
    apparmor_parser -r -T -W /etc/apparmor.d/bharat-browser 2>/dev/null || true
fi

# Register as a selectable default browser
if [ "$1" = "configure" ]; then
    update-alternatives --install /usr/bin/x-www-browser x-www-browser /usr/bin/bharat-browser 40
    update-alternatives --install /usr/bin/gnome-www-browser gnome-www-browser /usr/bin/bharat-browser 40
fi
...
# Pre-removal script for Bharat Browser
set -e

# Unregister as default browser
if [ "$1" = "remove" ] || [ "$1" = "deconfigure" ]; then
    update-alternatives --remove x-www-browser /usr/bin/bharat-browser 2>/dev/null || true
    update-alternatives --remove gnome-www-browser /usr/bin/bharat-browser 2>/dev/null || true
fi

# Unload AppArmor profile before files are removed
if command -v apparmor_parser >/dev/null 2>&1 && [ -f /etc/apparmor.d/bharat-browser ]; then
    apparmor_parser -R /etc/apparmor.d/bharat-browser 2>/dev/null || true
fi
...
def create_apparmor_profile(ctx: Context, apparmor_dir: Path) -> None:
    ...
    profile_content = f"""# AppArmor profile for Bharat Browser
...
profile bharat-browser /usr/lib/bharat-browser/{ctx.BROWSEROS_APP_NAME} flags=(unconfined) {{
  userns,

  include if exists <local/bharat-browser>
}}
"""

    profile_path = Path(join_paths(apparmor_dir, "bharat-browser"))
...
def create_metainfo_file(ctx: Context, metainfo_dir: Path) -> None:
    ...
    and other AppStream-aware tools can display Bharat Browser in their catalogs.
    """
...
<component type="desktop-application">
  <id>bharat-browser.desktop</id>
  <launchable type="desktop-id">bharat-browser.desktop</launchable>
  <name>Bharat Browser</name>
  <developer id="com.bharatbrowser">
    <name>Bharat Browser Team</name>
  </developer>
  ...
  <url type="homepage">https://www.bharatbrowser.com/</url>
  <url type="bugtracker">https://github.com/bharatbrowser-ai/BharatBrowser/issues</url>
  <url type="help">https://docs.bharatbrowser.com/</url>
  <description>
    <p>
      Bharat Browser is a privacy-focused web browser built on Chromium,
      designed for modern web browsing with AI capabilities.
...
"""
    ...
    metainfo_path = Path(join_paths(metainfo_dir, "bharat-browser.metainfo.xml"))
...
def prepare_debdir(ctx: Context, debdir: Path) -> bool:
    ...
    lib_dir = join_paths(debdir, "usr", "lib", "bharat-browser")
    ...
    # Create desktop file
    create_desktop_file(apps_dir, "/usr/bin/bharat-browser")
